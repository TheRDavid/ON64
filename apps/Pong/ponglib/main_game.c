...
{
    int accX = 0;
    while(accX == 0) accX = rand() % 9 - 4;
    int accY = rand() % 9 - 4;
    ball->x = (ZERO_X + RES_X) / 2;
    ball->y = (ZERO_Y + RES_Y) / 2;
...
int calc_main_game_screen(struct SCREEN *screen)
{
    ...
    if(player1.y + player1.acceleration + player1.height < MAX_Y && player1.y + player1.acceleration > 0)
        player1.y += player1.acceleration;

    if(player2.y + player2.acceleration + player2.height < MAX_Y && player2.y + player2.acceleration > 0)
    ...
    if(pong_ball.x - pong_ball.radius < player1.x + player1.width && pong_ball.x + pong_ball.radius > player1.x && pong_ball.y + pong_ball.radius > player1.y && pong_ball.y - pong_ball.radius < player1.y + player1.height)
    {
        pong_ball.acceleration_x *= -1;
        pong_ball.acceleration_y += player1.acceleration;
    }

    if(pong_ball.x - pong_ball.radius < player2.x + player2.width && pong_ball.x + pong_ball.radius > player2.x && pong_ball.y + pong_ball.radius > player2.y && pong_ball.y - pong_ball.radius < player2.y + player2.height)
    {
        pong_ball.acceleration_x *= -1;
...
int init_main_game_screen(struct SCREEN *screen, display_context_t display)
{
    ...
    player2 = (struct player) { .width = 10, .height = 80, .x = ZERO_X + RES_X / 8 * 7, .y = ZERO_Y + RES_Y / 3 * 2, .color = screen->params[3] };
    screen->params[4] = FALSE; // game over
}