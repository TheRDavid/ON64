...
int draw_main_menu(struct SCREEN *screen)
{
    ...
    for(int i = 0; i < main_menu_num_items; i++)
    {
        ...
        graphics_draw_text(screen->display, (ZERO_X + RES_X) / 2 - strlen(main_menu_items[i]) / 2 * 10, 20 + i * 35, main_menu_items[i]);
    }
    char livesText[35];
    snprintf(livesText, 35, "< %d >", screen->params[1]);
    graphics_draw_text(screen->display, (ZERO_X + RES_X) / 2 + 50, 55, livesText); 
    gfx_draw_rectangle(screen->display, (ZERO_X + RES_X) / 2 + strlen(main_menu_items[2]) / 2 * 10 - 20, 85, 60,20, player_colors[screen->params[2]], TRUE);
    gfx_draw_rectangle(screen->display, (ZERO_X + RES_X) / 2 + strlen(main_menu_items[2]) / 2 * 10 - 20, 120, 60,20, player_colors[screen->params[3]], TRUE);
...
int exit_main_menu(struct SCREEN *screen)
{
    ...
    main_game_screen->params[1] = screen->params[1];      
    main_game_screen->params[2] = screen->params[2];    
    main_game_screen->params[3] = screen->params[3];   
