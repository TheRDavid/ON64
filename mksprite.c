...
int merged = 0, count = 0;

void write_value( uint8_t *colorbuf, FILE *fp, int bitdepth, int y, int x)
{
            ...
            case PNG_COLOR_TYPE_RGB:
                ...
                for( int j = 0; j < height; j++)
                {
                    for( int i = 0; i < width; i++ )
                    {
                        ...
                        write_value( buf, op, depth, j, i);
                    }
                }

                break;
            case PNG_COLOR_TYPE_RGB_ALPHA:
		fprintf(stderr, "It's an image with alpha channel\n");
                /* Easy, just dump rows or convert */
                for( int row = 0; row < height; row++ )
                {
                    for( int col = 0; col < width; col++ )
                    {
                        write_value( &row_pointers[row][col * 4], op, depth, row, col);
                    }
                }
...
int main( int argc, char *argv[] )
{
    ...
    {
	printf("converting to 2 compressed 8-bit images");
        bitdepth = BITDEPTH_16BPP;
	merged = 1;
    }
    else
